...
-- Author: AdrienRmd
-- Published: mod-fnXiRjawrEb
-- Status: Stable
-----------------------------------------------------------

print("[age] script loaded")

-- ============================================================================
-- 1. VALUES -- EDIT HERE (defaults = user balanced values)
...
local europe_age = {
    enabled = true,
    time  = 90,
    worker_requirements_addition = 20,
    food  = { amount = 0 },
    wood  = { amount = 450 },
    iron  = { amount = 0 },
    gold  = { amount = 0 },
...
local iron_age = {
    enabled = true,
    time  = 120,
    worker_requirements_addition = 30,
    food  = { amount = 0 },
    wood  = { amount = 0 },
    iron  = { amount = 500 },
    gold  = { amount = 0 },
...
local western_europe_age = {
    enabled = true,
    time  = 150,
    worker_requirements_addition = 40,
    food  = { amount = 0 },
    wood  = { amount = 2500 },
    iron  = { amount = 1000 },
    gold  = { amount = 0 },
...
local india_IR2_age = {
    enabled = true,
    time  = 240,
    worker_requirements_addition = 2,
    food  = { amount = 12000 },
    wood  = { amount = 8000 },
    iron  = { amount = 5000 },
    gold  = { amount = 0 },
...
for _, t in ipairs(temples) do
    for _, w in ipairs(t.works) do
        local ok, err = pcall(function()
            ...
            if work == nil then
                print(string.format("[age] WARN: unit %d has no work slot %d (%s) — skipped",
                    t.id, w.wid, w.res))
                skipped = skipped + 1
                return
            end
            ...
            if v.worker_requirements_addition then
                ...
                if req == nil then
                    print(string.format(
                        "[age] WARN: unit %d work %d has no requirements.unit — worker requirement skipped",
                        t.id, w.wid))
                else
                    if req.size == 0 then
                        req.f_create()
            ...
            applied = applied + 1
            print(string.format("[age] %s (unit %d, work %d = %s): %d s%s%s",
        ...
        if not ok then
            failed = failed + 1
            print(string.format("[age] ERROR: unit %d work %d (%s): %s",
...
print(string.format("[age] done: %d applied, %d skipped, %d failed",
    applied, skipped, failed))

addMod({
    onStart = function(var)
        print("[age] onStart: research changes were applied at script load")
    end,
})

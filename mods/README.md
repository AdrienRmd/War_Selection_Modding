...
How to read the table: **Mod** links to the mod's folder, **Description** tells you what it changes in one line, **Status** says how mature it is (*Stable* = safe for normal games, *WIP* = work-in-progress), and **Mod ID** is the in-game ID to add an already-published mod directly to your map (see [how to add a mod by ID](../docs/modding-guide/installation.md#part-3--add-an-existing-already-published-mod-to-your-map) — `—` means not published).
...
| [diplomacy](diplomacy/) | Player-to-player diplomacy: 3 new buttons — request alliance, declare neutral (peace), declare enemy (war), plus a shared allied victory. | Stable | 3 scripts — [see README](diplomacy/#mod-ids) |
